...
then 
    echo -e "$R ERROR: user doesnot have root access Please access via root user $N" | tee -a $LOG_FILE #indentation is must **********
    exit 1
else
...
# $? - here it is command used to check the status of exit code status  whether it is success or failure sucess = 1 and failure 0-127   
    ...
    echo -e "Nginx is already installed ..... $Y nothing is needed to install $N" | tee -a $LOG_FILE     
fi     
...
# $? - here it is command used to check the status of exit code status  whether it is success or failure sucess = 1 and failure 0-127    
    ...
    echo -e "python3 is already installed ..... $Y nothing is needed to install $N" | tee -a $LOG_FILE      
fi    